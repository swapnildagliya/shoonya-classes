  ...
  var WS_STYLE_DATA = {
    ...
    "Contemporary dance": {
      ...
      "passPricing": {
        "label": "Class passes",
        "cta": "Book a pass →",
        "note": "One pass covers Tono’s four morning classes — spend it on ballet, contemporary, or any mix of the two. It does not cover other ballet classes at Shoonya. No semester commitment. A pass is valid until the end of this term. In the registration form choose <strong>Festivals &amp; Events</strong>, then this class, then your pass size."
      },
      "mergeStyles": [
        {
          "style": "Ballet",
...
function wsDropinSectionHtml(prefix, note, cfg) {
  ...
  // 2026-08-17: customer-facing wording is "session pass", never "drop-in" / "losse les".
  // These are 3- and 5-session packs (beurtenkaart), not turn-up-any-time drop-ins, and the
  // old label mis-sold what is actually bought. CSS class names keep the `dropin` stem on
  // purpose — they are internal identifiers wired to WS_STYLE_DATA and the pasted blocks.
  const label = cfg.label || 'Session passes';
  const cta   = cfg.cta   || 'Book a session pass →';
  ...
  // 2026-09-16 (Swapnil): a 3/5-session pass is bound to ONE class and expires at the end of the
  // semester it was bought in. Stated on every session-pass block so the question does not reach the
  // inbox. Pro-training class passes (cfg.label set) run on their own terms and do not get it.
  const validity = cfg.label ? '' : ' A pass is valid for this class only, until the end of the current semester.';
  return `<div class="${p}-dropin-section">
            <div><p class="${p}-dropin-label">${label}</p>
            <p class="${p}-dropin-note">${note || WS_DROPIN_NOTE}${validity}</p></div>
